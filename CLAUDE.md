...
**Status**: Phase 6 (frontend) complete — the full stack is live end-to-end. `/auth/{register,login,refresh,logout}` are live (Phase 2): bcrypt (cost 12) password hashing, HS256 JWT access/refresh pair issuance, session create + rotation with token-family reuse detection (a reused/revoked refresh token revokes its whole family), Redis-backed access-token blacklist and login rate limiting (5 attempts / 15 min), and best-effort audit logging of `user.register`/`user.login`. `RequireAuth`/`RequireOrg` guard the org routes (Phase 3): `POST /organizations` (create + owner membership in a tx), `GET /organizations` (caller's memberships with embedded org), `GET /organizations/members` (active org's member roster — added in Phase 6 for the frontend, not present in the source app; see `docs/03` "Deviations resolved during Phase 6"), `POST /organizations/invite` (role check + `max_members` plan-limit enforcement), `DELETE /organizations/members/:userId` (role check + cannot-remove-owner) — with best-effort `org.created`/`org.member.invited` audit logging. `RequirePermission(action)` now guards permission-checked routes with `*`/exact/`resource:*` semantics and owner bypass (no contract route currently uses it — parity with the source macro, exercised by unit tests). `/rbac/{roles,assign}` (create/list/update-permissions role CRUD-lite + role assignment), `/subscription`/`/subscription/assign` (get with embedded plan, upsert), and `GET /audit-logs` (filterable by `userId`/`action`, capped `limit`) are all live and `RequireOrg`-guarded. `GET /plans` (`RequireAuth`-only, not org-scoped) lists all plans — added in Phase 6 so the frontend subscription page can populate a plan picker, since plan ids are server-generated UUIDs with no other way to discover them; see `docs/03` "Deviations resolved during Phase 6". Phase 5 added: every route documented at `/swagger` (swaggo-generated, spec committed to `apps/backend/docs/`); a distroless production image (`gcr.io/distroless/static-debian12:nonroot`) with a dedicated `cmd/healthcheck` binary since distroless has no shell for `HEALTHCHECK`; `golangci-lint` (v2 config schema) wired into `make lint` and CI; k8s manifests ported to `k8s/` (env parity: `APP_ENV`/`APP_NAME`, not `NODE_ENV`); and CI gained a `lint` job plus a `docker` build job, with a new `release.yml` pushing to ghcr on CI success against `main`. Phase 6 added the Next.js dashboard (`apps/frontend/`, App Router + shadcn/ui + TanStack Query): pages for every module (auth, organizations + switcher, members, RBAC roles, audit logs, subscription), a client-token/single-flight-refresh auth model, and a runtime reverse proxy (`app/api/[...path]/route.ts`, not a build-time `next.config.ts` rewrite — see `docs/03` "Deviations resolved during Phase 6") so the browser only ever calls same-origin `/api/*` and no backend CORS is needed. `compose.yaml`'s `web` service and the CI `frontend` job (lint/typecheck/test/build) are both live. See [`README.md`](README.md) for the quickstart to run it, [`apps/frontend/README.md`](apps/frontend/README.md) for frontend-specific details. `docs/` holds the analysis and migration plan. Read `docs/` before implementing anything.
...
- Frontend never calls the backend cross-origin: `app/api/[...path]/route.ts` is a runtime proxy reading `BACKEND_URL` per-request, not a `next.config.ts` `rewrites()` entry (those resolve at build time, not container-runtime — see `docs/03` "Deviations resolved during Phase 6"). **No CORS middleware exists on the backend and none should be added** — the browser only ever calls same-origin `/api/*`.
...
No process manager runs backend + frontend together — `make api` and `make web` are two terminals. Regenerating sqlc code requires the `sqlc` CLI (`go install github.com/sqlc-dev/sqlc/cmd/sqlc@latest`); building/running the API does not. Full containerized stack: `docker compose up -d --build` (db, redis, api, web).
